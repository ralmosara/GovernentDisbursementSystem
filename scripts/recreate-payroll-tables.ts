/**
 * Recreate Payroll Tables
 * Drops existing payroll tables and recreates them with the correct schema
 */
...
async function recreatePayrollTables() {
  console.log('Recreating Payroll Tables\n');
  ...
  try {
    ...
    // Drop existing payroll tables in reverse order (due to foreign keys)
    console.log('Dropping existing payroll tables...');
    await connection.query('SET FOREIGN_KEY_CHECKS=0');
    await connection.query('DROP TABLE IF EXISTS thirteenth_month_pay');
    await connection.query('DROP TABLE IF EXISTS remittances');
    await connection.query('DROP TABLE IF EXISTS payroll_adjustments');
    await connection.query('DROP TABLE IF EXISTS employee_deductions');
    await connection.query('DROP TABLE IF EXISTS payroll_transactions');
    await connection.query('DROP TABLE IF EXISTS payroll_periods');
    ...
    console.log('[SUCCESS] All payroll tables recreated successfully!\n');
    process.exit(0);
  } catch (error) {
    console.error('[ERROR] Failed to recreate payroll tables:', error);
    if (connection) await connection.end();
    process.exit(1);
  }
}

recreatePayrollTables();
